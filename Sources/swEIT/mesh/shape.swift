...
func ellipse(pts : [[Double]], pc : [Double]=[], ab:[Double]=[])-> [Double]{
    ...
    */
    ...
    var pdiff : [[Double]] = []
    for pt in pts{
        pdiff.append([(pt[0]-pc2[0])/ab[0],(pt[1]-pc2[1])/ab[1]])
    }
...
func fix_points_fd(fd: ([[Double]],[Double]?,Double?) -> [Double], n_el:Int=16, pc:[Double]=[]) -> [[Double]]{
    /*
    ...
    */
    ...
    while !c{
        // project on fd
        pts_new = edge_project(pts: pts, fd: fd)
        // project on rays
