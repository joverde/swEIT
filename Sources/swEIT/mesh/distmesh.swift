...
public class DISTMESH{
    ...
    public func triangulate(){
        ...
        var temp : [[Int]] = []
        var t : [[Int]] = []
        for (i,n) in self.fd(pmid, [], 1.0).enumerated(){
            if n < -self.geps{
                temp.append(tri[i])
            }
        }
    ...
    func move_p(Ftot:[[Double]])-> Bool{
        ...
        if pvals.count > 0 {
            for (i,n) in zip(edge_project(pts: p_edge, fd: self.fd, h0: self.geps),pvals){
                self.p[n][0] = i[0]
                self.p[n][1] = i[1]
            }
        }
...
public func build(fd : @escaping ([[Double]],[Double]?,Double?) -> [Double], fh : @escaping ([[Double]]) -> [Double], pfix:[[Double]]=[], bbox:[[Double]]=[], h0:Double=0.1, densityctrlfreq:Int=10, maxiter:Int=500, verbose:Bool=false) -> ([[Double]], [[Int]]){
    ...
    */
    ...
    // initialize distmesh
    let dm = DISTMESH(fd: fd,fh: fh,h0: h0,p_fix: pfix,bbox: bbox,density_ctrl_freq: densityctrlfreq,deltat: g_deltat,dptol: g_dptol,ttol: g_ttol,Fscale: g_Fscale,verbose:verbose)
